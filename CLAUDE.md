...
Independent **HubSpot consulting practice** site for a solo consultant repositioning from Salesforce to HubSpot. Content is intentionally minimal: one homepage, `/services`, `/about`, `/notes` + `/notes/[slug]`. No case studies, no tag routes, no comments, no search, no newsletter, no client logos, no testimonials, no privacy/terms/llms.txt (removed). There are comment-only stubs for a logo strip and testimonial section, ready to switch on when honest content exists.
...
**Homepage** (`src/pages/index.astro`), 8 marked sections `§01`–`§08` (plus footer):
...
`{{NAME}}`, `{{DOMAIN}}`, `{{EMAIL}}`, `{{TARGET_MARKET}}`, `{{YEARS_SF}}`, `{{ENV_DESCRIPTORS}}`, `{{CERTS}}`, `{{PORTAL_ID}}`, `{{PORTAL_REGION}}`, `{{MEETINGS_EMBED_URL}}`, `{{FORM_ID_AUDIT}}`, `{{FORM_ID_LOW_FRICTION}}`, and per-offer `{{OFFER_N_NAME|WHO|DELIVERED|DURATION|PRICE}}` × 3.

Socials are filled: `SITE_METADATA.socials.linkedin`, `.twitter`, `.youtube`.
